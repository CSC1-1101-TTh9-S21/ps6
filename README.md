...
* Delete your `part1.py1` code for `solvethecipher(text)` and rewrite it as follows. The most frequent letter English texts is `E`. Therefore, one of letters in the encoded text that is among the most frequent is likely to be `E`! Instead of trying every possible shift size, you will strategically choose the shift size that will map the most frequent character in the encoded text to `E`, and then submit that to `checkgoodness()`. If `checkgoodnes()` returns false, choose the shift size that will map the second most frequent character in the encoded text to `e`, and submit to `checkgoodness()`. Keep doing this until `checkgoodness()` returns `True`, then return the encoded text, the shift size, and the number of shifts you had to try before getting a solution.
